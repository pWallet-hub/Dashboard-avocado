...
import { ArrowLeft, ShoppingCart, Star, CheckCircle, Shield, HardHat, AlertTriangle, Layers, Heart, Filter, X, Smartphone } from 'lucide-react';
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {cartSummary.isEmpty ? (
              <div className="flex h-full flex-col items-center justify-center text-center">
                ...
                <p className="text-gray-500">Add some safety equipment to get started!</p>
              </div>
            ) : (
...
export default function SafetyProtectionEquipment() {
  ...
  const ProductModal = ({ product, onClose }) => (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 animate-fadeIn">
      <div className="bg-white rounded-2xl p-6 max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border-2 border-green-200 animate-slideUp">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-3xl font-bold text-green-900">{product.name}</h2>
          <button
            onClick={onClose}
            className="p-2 hover:bg-green-100 rounded-full transition-all duration-200 hover:rotate-90"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-4">
            <div className="relative group">
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-green-700/20 to-transparent rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
            </div>
            <div className="flex space-x-2">
              {[1, 2, 3, 4].map(i => (
                <img 
                  ...
                  className="w-20 h-20 object-cover rounded-lg border-2 border-green-200 hover:border-green-500 cursor-pointer transition-all duration-200 hover:scale-110"
                />
              ))}
            </div>
          </div>
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex items-center space-x-3">
                <span className="text-3xl font-bold text-green-700">{product.price.toLocaleString()} RWF</span>
                {product.originalPrice && (
                  <span className="text-xl text-gray-400 line-through">{product.originalPrice.toLocaleString()} RWF</span>
                )}
              </div>
            ...
            <div className="space-y-3">
              <h3 className="text-lg font-semibold text-green-900">Safety Features:</h3>
              ...
              <button 
                ...
                className={`flex-1 py-3 px-6 rounded-2xl font-semibold transition-all duration-300 ${
                  justAdded === product._id
                    ...
                    ? 'bg-black text-white'
                    : product.inStock
                    ? 'bg-gradient-to-r from-green-700 to-black text-white hover:from-black hover:to-green-700 hover:scale-105 shadow-lg hover:shadow-xl'
                    : 'bg-gray-300 text-gray-500 cursor-not-allowed'
                }`}
              >
                {addingToCart === product._id ? (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-2xl p-6 text-center shadow-lg border border-green-200 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 animate-slideInLeft">
            ...
            <p className="text-gray-600">Pesticide Protection</p>
          </div>
          <div className="bg-white rounded-2xl p-6 text-center shadow-lg border border-green-200 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 animate-slideInLeft" style={{ animationDelay: '0.1s' }}>
            ...
            <p className="text-gray-600">For Rwanda Farms</p>
          </div>
          <div className="bg-white rounded-2xl p-6 text-center shadow-lg border border-green-200 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 animate-slideInLeft" style={{ animationDelay: '0.2s' }}>
            ...
            <p className="text-gray-600">Safety in Fields</p>
          </div>
            ...
            {filteredProducts.length > 0 ? (
              <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
                {filteredProducts.map((product, index) => (
                  <div
                    key={product._id}
                    className="group bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 border border-green-200 hover:border-green-300 transform hover:-translate-y-2 animate-slideInUp"
                  ...
                  >
                    <div className="relative overflow-hidden">
                      <img
                        ...
                        style={{ background: '#e5fbe5' }}
                      />
                      <div className="absolute inset-0 bg-gradient-to-t from-green-700/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                      {product.discount && (
                        <div className="absolute top-4 left-4 bg-gradient-to-r from-green-700 to-black text-white px-3 py-1 rounded-full text-sm font-semibold animate-pulse">
                          {product.discount}% OFF
                        </div>
                      )}
                      <div className="absolute top-4 right-4 space-y-2">
                        ...
                        >
                          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12c0-1.1-.9-2-2-2s-2 .9-2 2c0 .74.4 1.38 1 1.72V21h2v-7.28c.6-.34 1-.98 1-1.72zm-3-8a8 8 0 100 16 8 8 0 000-16zm0 14a6 6 0 110-12 6 6 0 010 12z" />
                          </svg>
                        </button>
                      </div>
                      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-green-700/80 to-transparent p-6 transform translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                    ...
                    <div className="p-6 bg-gradient-to-br from-white to-green-50">
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center space-x-2">
                          <span className="text-2xl font-bold text-green-700">{product.price.toLocaleString()} RWF</span>
                          {product.originalPrice && (
                            <span className="text-lg text-gray-400 line-through">{product.originalPrice.toLocaleString()} RWF</span>
                          )}
                        </div>
