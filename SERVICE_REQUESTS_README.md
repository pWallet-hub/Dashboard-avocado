...
i shared u two documentation one is for agent information and other is general documentation contain multiple different endpoint so in thi general documentation need see on harvestig request endpoint used and property evaluation only other remain as usual the is : need to display the farmer be  visible agent dashboard but based on territory of agent where check thedistrict and sector if match with the the territory then display those user i agent corresponding for example there is farmer have has farme location based in nyarugenge district sector nyamirambo  and also there is agent have those territory as one of the the territory then make displayed in agent match with those location and also agent will be required to make request service  of farmer but if it request must visible as agent generally display the request body as usual causeit send request information and information of farmer but add some information agent when it done with agent but if it is farmer no need of agent information mean it is not mandatory

help me to implement other change u did before reverse the original only that endpoint